...
function Get-SkinMetadataVersion {
    ...
    $settingsIni = Get-BlockHudSettingsIniPath -Root $Root
...
function Get-TargetRoot {
    if ([string]::IsNullOrWhiteSpace($TargetRoot)) {
        ...
        if (Get-Command Resolve-BlockHudSkinRoot -ErrorAction SilentlyContinue) {
            return (Resolve-BlockHudSkinRoot -StartPath $toolsRoot)
        }
        foreach ($candidate in @(
            $toolsRoot,
            (Join-Path $toolsRoot '..'),
            ...
            $resolvedCandidate = Resolve-FullPath -Path $candidate -AllowMissing
            if ((Test-Path -LiteralPath $resolvedCandidate -PathType Container) -and (Test-SkinRoot -Root $resolvedCandidate)) {
                return $resolvedCandidate
            }
        }
        return (Resolve-FullPath -Path (Join-Path $toolsRoot '..\..'))
    }
...
function Get-ImportHelperPath {
    ...
    Get-BlockHudRuntimeToolPath -Root $Root -RelativeToolPath 'ImportFromOldVersion.ps1'
}
...
function Get-VersionCatalogHelperPath {
    ...
    Get-BlockHudRuntimeToolPath -Root $Root -RelativeToolPath 'GetVersionReleaseCatalog.ps1' -AllowMissing
}
...
function Get-VersionReleaseInstallHelperPath {
    ...
    Get-BlockHudRuntimeToolPath -Root $Root -RelativeToolPath 'InstallVersionRelease.ps1' -AllowMissing
}
