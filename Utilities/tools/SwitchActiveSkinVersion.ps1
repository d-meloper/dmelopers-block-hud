...
function Invoke-DeactivateConfigList {
    ...
    foreach ($spec in $Specs) {
        $resolvedRelativePath = ConvertTo-BlockHudConfigRelativePath -Root $Root -LegacyRelativePath ([string]$spec.RelativePath)
        $configName = Get-ConfigName -RootConfigName $rootConfigName -RelativeConfigPath $resolvedRelativePath
...
function Invoke-ActivateZPosBootstrap {
    ...
    $spec.RelativePath = Get-BlockHudBootstrapRelativePath -Root $Root
    $rootConfigName = Get-RootConfigName -Root $Root
...
function Invoke-BestEffortRollback {
    ...
    $spec = Get-ZPosBootstrapSpec
    $spec.RelativePath = Get-BlockHudBootstrapRelativePath -Root $CurrentRoot
    $selectedRootConfig = Get-RootConfigName -Root $SelectedRoot
    foreach ($relativePath in @('Bootstrap', 'Bootstrap\ZPosBootstrap')) {
        $relativePath = ConvertTo-BlockHudConfigRelativePath -Root $SelectedRoot -LegacyRelativePath $relativePath
