...
$defaultItemSkinLayoutCommonPath = Join-Path $PSScriptRoot 'SkinLayout.Common.ps1'
if (-not (Get-Command Resolve-BlockHudSkinRoot -ErrorAction SilentlyContinue) -and (Test-Path -LiteralPath $defaultItemSkinLayoutCommonPath -PathType Leaf)) {
    . $defaultItemSkinLayoutCommonPath
}

function Get-DefaultItemLocalizationSkinRoot {
    ...
    $candidate = [System.IO.Path]::GetFullPath($ScriptRoot)
    for ($depth = 0; $depth -lt 8; $depth++) {
        if (Test-Path -LiteralPath (Join-Path $candidate '@Resources') -PathType Container) {
            return $candidate
        }
...
function Get-DefaultItemLocalizationRecords {
    @(
        ...
        [PSCustomObject]@{ DataSet = 'Hotbar'; Section = 'Slot09'; Image = 'Potion_Water.png'; Qty = '0'; Action = 'https://linktr.ee/dmeloper.dev'; ActionAliases = @('https://litt.ly/dmeloper', 'https://litt.ly/dmeloper.dev'); LabelKey = 'Loc_DefaultItem_DeveloperLinktree'; InventoryLabel = $false; AlwaysUpdate = $false }
...
function Test-DefaultItemLocalizationIdentity {
    ...
    $action = [string]$Variables[$base + 'Action']
    $acceptedActions = New-Object 'System.Collections.Generic.List[string]'
    [void]$acceptedActions.Add([string]$Record.Action)
    if ($null -ne $Record.PSObject.Properties['ActionAliases']) {
        foreach ($alias in @($Record.ActionAliases)) {
            if (-not [string]::IsNullOrWhiteSpace([string]$alias)) {
                [void]$acceptedActions.Add([string]$alias)
            }
        }
    }

    ([string]$Variables[$base + 'Image'] -ceq [string]$Record.Image) -and
    (@($acceptedActions.ToArray()) -ccontains $action) -and
