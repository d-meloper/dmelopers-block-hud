...
function Get-ImportFromOldVersionDefaultTargetRoot {
    ...
    if (-not [string]::IsNullOrWhiteSpace($entrypointRoot)) {
        ...
        $resolvedEntrypointRoot = [System.IO.Path]::GetFullPath($entrypointRoot)
        foreach ($relative in @('..\..', '..')) {
            $candidate = [System.IO.Path]::GetFullPath((Join-Path $resolvedEntrypointRoot $relative))
...
function Add-ItemConfirmBeforeRunBackfill {
    param(
        [Parameter(Mandatory = $true)]
...
function Ensure-CacheFormat2 {
    ...
    $cachePath = Join-RootPath -Root $TargetRoot -RelativePath 'HUD\Settings\Cache.inc'
...
function Invoke-Migration {
    ...
    $sourceEditorDraftPath = Join-RootPath -Root $sourceData -RelativePath 'EditorDraft.inc'
