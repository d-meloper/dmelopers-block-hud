...
if (-not [System.IO.File]::Exists($scriptPath)) {
    Write-Host 'Snapshot import script was not found:'
...
if ([string]::IsNullOrWhiteSpace($status)) {
    Write-Host ''
    Write-Host 'Snapshot import failed because the helper did not emit DMEL_STATUS.'
...
if ($status -ieq 'CANCEL') {
    Write-Host ''
    Write-Host 'Snapshot import was cancelled. No changes were applied.'
...
if ($status -ieq 'OK' -and $exitCode -eq 0) {
    Write-Host ''
    Write-Host 'Snapshot import completed successfully. Review the log path printed above.'
    Show-Pause
    exit 0
}

Write-Host ''
Write-Host 'Snapshot import failed. Review the log path printed above.'
