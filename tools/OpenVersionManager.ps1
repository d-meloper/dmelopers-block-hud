...
function Wait-VersionManagerLaunchShown {
    ...
    return [PSCustomObject]@{
        Status = 'WARN'
        Message = ("Skin manager launch was started, but the window did not report shown before the confirmation timeout. expectedToken={0}; observedToken={1}; observedStatus={2}" -f [string]$ExpectedLaunchToken, $lastToken, $lastStatus)
        ObservedStatus = $lastStatus
        ObservedToken = $lastToken
    }
}
...
function Get-VersionManagerLogView {
    ...
    return [PSCustomObject]@{
        ...
        Text = if ($blocks.Count -gt 0) { [string]::Join("`r`n`r`n", $blocks) } else { [string](T 'Helper_VersionManager_Log_Empty' 'No skin-wide logs yet.') }
    }
}
...
function Confirm-Dialog {
    ...
    [System.Windows.Forms.MessageBox]::Show(
        $Owner,
        $Message,
        $(if ($Title) { $Title } else { T 'Helper_VersionManager_WindowTitle' 'Skin manager' }),
...
function Invoke-CheckLatestReleaseApi {
    ...
    $uri = 'https://api.github.com/repos/{0}/{1}/releases/latest' -f $Config.Owner, $Config.Repo
...
function Invoke-CheckLatestRelease {
    ...
    try {
        ...
        if (Test-GitHubApiRateLimitException -Exception $_.Exception) {
            $response = Invoke-CheckLatestReleaseFallback -Config $Config
        }
        else {
            throw
        }
...
function Start-VersionManagerLauncherForSupportedRoot {
    ...
    Write-Log ("Selected root version v{0} predates Skin manager support; keeping the manager closed after switch: {1}" -f $versionText, $ResolvedTargetRoot) 'INFO'
    return $false
}
...
function Start-VersionManager {
    ...
    $form = New-Object System.Windows.Forms.Form
    $form.Text = T 'Helper_VersionManager_WindowTitle' 'Skin manager'
    ...
    $currentInstallGroup.Text = T 'Helper_VersionManager_Install_CurrentGroup' 'Current install'
    ...
    $installButtons.UseVersion.Text = T 'Helper_VersionManager_Action_UseVersion' 'Use this version'
    ...
    $busyOverlayMessage.Text = T 'Helper_VersionManager_Busy_Default' 'Preparing the skin update. Please do not close this window.'
    ...
    $showBusyOverlay = {
        ...
        if ([string]::IsNullOrWhiteSpace($Message)) {
            $Message = T 'Helper_VersionManager_Busy_Default' 'Preparing the skin update. Please do not close this window.'
        }
    ...
    $installLatestVersion = {
        ...
        try {
            ...
            $cache = Get-UpdateCache -Root $ui.Root
            $latestComparableVersion = Convert-ToVersion -VersionText ([string]$cache.LatestVersion)
    ...
    $settingsLogClearButton.Add_Click({
        ...
        if (-not (Confirm-Dialog -Owner $form -Message (T 'Helper_VersionManager_Log_ClearConfirm' 'Delete all skin-wide logs and clear the current session log view?'))) {
            return
        }
...
try {
    ...
    if ($WindowSession) {
        ...
        if (-not [string]::IsNullOrWhiteSpace([string]$script:ResultPairs['DMEL_STATUS'])) {
            Set-ResultPairValue -Key 'DMEL_MESSAGE' -Value (T 'Helper_VersionManager_Result_ClosedAfterOperation' 'Skin manager closed after completing an operation.')
        }
        else {
            Set-ResultPairValue -Key 'DMEL_STATUS' -Value 'OK'
            Set-ResultPairValue -Key 'DMEL_MESSAGE' -Value (T 'Helper_VersionManager_Result_Closed' 'Skin manager closed.')
        }
    }
    else {
        ...
        if ([string]::Equals([string]$launchResult.Status, 'OK', [System.StringComparison]::OrdinalIgnoreCase)) {
            Set-ResultPairValue -Key 'DMEL_STATUS' -Value 'OK'
            Set-ResultPairValue -Key 'DMEL_MESSAGE' -Value (T 'Helper_VersionManager_Result_Launched' 'Skin manager opened.')
        }
        elseif ([string]::Equals([string]$launchResult.Status, 'ERROR', [System.StringComparison]::OrdinalIgnoreCase)) {
            ...
            if ([string]::IsNullOrWhiteSpace($launchErrorMessage)) {
                $launchErrorMessage = (T 'Helper_VersionManager_Result_OpenFailed' 'Skin manager could not be opened. Check the log file for details.')
            }
            ...
            if ([string]::IsNullOrWhiteSpace($launchWarningMessage)) {
                $launchWarningMessage = 'Skin manager launch confirmation timed out; Settings will keep watching for the window state.'
            }
    ...
    Set-ResultPairValue -Key 'DMEL_STATUS' -Value 'ERROR'
    Set-ResultPairValue -Key 'DMEL_MESSAGE' -Value (T 'Helper_VersionManager_Result_OpenFailed' 'Skin manager could not be opened. Check the log file for details.')
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($failedRoot) -and (Test-Path -LiteralPath $failedRoot -PathType Container)) {
            Save-VersionManagerLaunchState -Root $failedRoot -Status 'error' -LaunchTokenValue $LaunchToken -Message (T 'Helper_VersionManager_Result_OpenFailed' 'Skin manager could not be opened. Check the log file for details.')
        }
    }
    catch {
    }
