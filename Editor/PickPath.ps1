...
try {
    ...
    $errorMessage = 'The path picker could not be opened. Check the helper files and try again.'
    try {
        $uiText = Get-PickerUiText
        $errorTitle = [string]$uiText.ErrorTitle
        $errorMessage = [string](L 'Helper_PickPath_WrapperError' 'The path picker could not be opened. Check the helper files and try again.')
    }
    catch {
    }
