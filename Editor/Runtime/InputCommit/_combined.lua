...
local function setEditorVisibleState(visible)
    ...
    if rootConfig ~= '' then
        for _, configName in ipairs({ 'Hotbar', 'Inventory', 'InventoryBG', 'Editor' }) do
            local configPath = rootConfig .. '\\' .. configName
            if EditorIsRainmeterConfigActive == nil or EditorIsRainmeterConfigActive(configPath) then
                SKIN:Bang('!SetVariable', 'BlockHudEditorVisible', value, configPath)
            end
        end
    end
    if editorRoot and editorRoot ~= '' then
        SKIN:Bang('!WriteKeyValue', 'Variables', 'BlockHudEditorVisible', value, editorRoot .. 'Defaults\\Runtime\\incs\\InternalState.inc')
    end
end

local function isEditorVisibleStateEnabled()
    return trim(SKIN:GetVariable('BlockHudEditorVisible', '0')) ~= '0'
end
...
local function syncInventoryEditorModeBadge(openValue)
    ...
    local inventoryConfig = rootConfig .. '\\Inventory'
...
local function commitPickedProgramForLocator(pathValue, imageValue, locator, displayLabel)
    ...
    updateFieldAtLocator(locator, "Action", resolvedPath)
