...
function ApplyImagePixelation()
    ...
    if parsed.stage >= 3 then
        ...
        EditorImagePixelationAndPreview.bridge().log('Warning', 'Editor image pixelation original source was not found; keeping current p3 image.')
        return false
    end
    ...
    if not ok then
        EditorImagePixelationAndPreview.bridge().alert(message, 'ModalAlert_EditorPixelateFailed', message)
...
function HandleEditorPixelationComplete()
    ...
    if not result.accepted then
        ...
        EditorImagePixelationAndPreview.bridge().alert(result.message, 'ModalAlert_EditorPixelateFailed', result.message)
        return false
    end
    ...
    local message = EditorImagePixelationAndPreview.trim(result.message) ~= '' and result.message or 'Image pixelation failed.'
