...
이 공개 저장소는 스킨 배포를 위한 저장소입니다. 유지관리자가 준비한 릴리즈 브랜치는 실제 공개 릴리즈가 되기 전에 이곳에서 검토되며, 구현 변경은 먼저 유지관리자의 비공개 개발 작업 공간에서 준비됩니다.
...
릴리즈 승인 Pull Request는 `publish/v<version>` 브랜치를 사용하고, `public-export-approval` 검사를 통과해야 하며, 수동으로 merge된 뒤 GitHub Release tag와 assets가 확정됩니다.
