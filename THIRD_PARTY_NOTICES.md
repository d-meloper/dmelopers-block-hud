# Third-Party Notices

English | [한국어](THIRD_PARTY_NOTICES.ko-KR.md)

This project includes or credits third-party components and references.

## Mouse.dll

- Project: Mouse.dll
- Author: NighthawkSLO
- Source: https://github.com/NighthawkSLO/Mouse.dll
- Version used for packaging: v3.2.0

Mouse.dll is credited as a third-party Rainmeter plugin. It is not authored by DMeloper and is not relicensed by this project's MIT License.

## Galmuri

- Project: Galmuri
- Author: Lee Minseo
- Source: http://quiple.dev/font/galmuri
- License: SIL Open Font License 1.1

Galmuri is credited as a third-party font. The Galmuri project states that it is available under the SIL Open Font License 1.1.

## Rainmeter

- Project: Rainmeter
- Source: https://www.rainmeter.net/

DMeloper's Block HUD is a Rainmeter skin and requires Rainmeter to run.

## Minecraft Notice

This skin is an unofficial desktop customization project inspired by Minecraft-style inventory and hotbar UI.

This project is not affiliated with, endorsed by, or sponsored by Mojang Studios or Microsoft. Minecraft and related trademarks, names, images, UI concepts, and copyrights belong to their respective owners.

The included item images are intentionally modified and are not distributed as original Minecraft assets. Users are responsible for ensuring they have the right to use any replacement assets they apply manually.
