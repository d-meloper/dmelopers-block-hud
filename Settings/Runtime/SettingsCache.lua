return function(app)
    ...
    function methods.CancelPendingLoad()
        ...
        if state.pendingLoadHelperRunning == true then
            ...
            methods.clearPendingLoadState({
                abandonActiveHelperReason = 'cancelled',
                clearIgnoredHelper = false,
            })
        else
            methods.clearPendingLoadState()
    ...
    function methods.handleLegacyImportHelperResult(values)
        ...
        if #details == 0 then
            details[#details + 1] = status == 'CANCEL' and methods.localize('Settings_Notice_LegacyCanceled', 'Old-data import was cancelled.') or methods.localize('Settings_Notice_LegacyFailed', 'Old-data import failed. Check the log folder for details.')
        end
